...
public class TestByteBuffer {
    ...
    public static void main(String[] args) {
        ...
        try (FileChannel channel = new FileInputStream("data.txt").getChannel()) { // Java7语法
            // 准备缓冲区
...
```
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 读取到的字节数 10
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 1
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 2
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 3
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 4
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 5
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 6
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 7
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 8
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 9
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 0
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 读取到的字节数 3
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 a
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 b
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 实际字节 c
16:35:32 [DEBUG] [main] c.i.n.c.TestByteBuffer - 读取到的字节数 -1
```
...
/*
  ...
  class java.nio.DirectByteBuffer  - 直接内存，读写效率高（少一次拷贝），不会受 GC 影响，分配的效率低，使用不当可能造成内存泄漏
*/
```
...
public static void main(String[] args) throws IOException {
    ...
    Files.walkFileTree(path, new SimpleFileVisitor<Path>(){
        ...
        public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) 
            ...
            dirCount.incrementAndGet(); // 文件夹数量
...
while (true) {
    ...
    SocketChannel sc = ssc.accept(); // 阻塞方法，线程停止运行，它要等待客户端连接建立
    log.debug("connected... {}", sc);
    channels.add(sc);
    for (SocketChannel channel : channels) {
        // 5. 接收客户端发送的数据
        log.debug("before read... {}", channel);
        channel.read(buffer); // 阻塞方法，线程停止运行，等待读入数据，没有数据就会阻塞
        // 在客户端使用DEBUG模式的Evaluate执行发送数据的表达式：
...
while (true) {
    ...
    if (sc != null) {
        log.debug("connected... {}", sc);
        sc.configureBlocking(false); // 非阻塞模式，影响下面的read方法
        channels.add(sc);
    }
    for (SocketChannel channel : channels) {
        // 5. 接收客户端发送的数据
...
// SelectionKey就是将来事件发生之后，通过它可以知道事件和哪个channel的事件
...
注意这里的异步，并不是之前的异步IO，而是指的是使用多线程完成方法调用和处理结果相分离。调用方法的线程和处理结果的线程是不同的。

### 1.2 Netty 的作者

<img src="IMG/Netty.assets/0005.png" title="" alt="" data-align="center">
...
new ServerBootstrap() // 启动器，负责组装netty组件，启动服务器
    .group(new NioEventLoopGroup()) // 1 该Group简单理解为：包含线程池 + 选择器Selector
    .channel(NioServerSocketChannel.class) // 2 选择 服务器的 ServerSocketChannel 实现
...
<img title="" src="IMG/Netty.assets/0040.png" alt="" data-align="inline">
...
对于NioEventLoopGroup的线程数来说：

```java
DEFAULT_EVENT_LOOP_THREADS = Math.max(1, SystemPropertyUtil.getInt(
                "io.netty.eventLoopThreads", NettyRuntime.availableProcessors() * 2));
// 如果指定了线程数，就会使用指定的。如果没有指定，或者指定的是0，就会看上述代码。
```
...
// DefaultEventLoopGroup可以处理普通任务和定时任务，不可以处理IO事件（NioEventLoopGroup可以）
...
* pipeline() 方法添加处理器
* write() 方法将数据写入，并不会立即刷出，可以调用flush方法将缓冲区中的数据发出去。
...
也就是说，如果我们没有添加sync()方法，就可能导致我们写入数据的时候，连接其实并没有真正的建立好。
...
**除了用 sync 方法可以让异步操作同步以外，还可以使用回调的方式异步处理结果：**
...
我们有如下需求：客户端源源不断给服务端发送数据，如果我们输入的是q，就代表结束输入。

但是我们还需要在channel关闭之后，进行一些善后操作，这时候就得使用到CloseFuture。
...
public class CloseFutureClient {
    public static void main(String[] args) throws InterruptedException {
        ...
        new Thread(()->{
            ...
            while (true) {
                ...
                if ("q".equals(line)) {
					...
					// log.debug("处理关闭之后的操作"); 
                    // 不能在这里善后 close是异步的，可能这行代码执行的时候，客户端还未真正关闭。
                    break;
                }
        ...
        closeFuture.sync(); // 阻塞住线程，等到channel关闭之后才执行后续
        log.debug("处理关闭之后的操作");*/
...
* 异步并没有缩短响应时间，反而有所增加
* 虽然看病人的总量其实没有变化，还是96，但是对单个医生来说，它的吞吐量就上升了。
* 合理进行任务拆分，也是利用异步的关键
...
一般而言，每创建一个slice都要调用retain方法让引用计数加1，使用完之后，调用对应的ByteBuf的release方法让引用计数减1。
...
这时调用 slice 进行切片，无参 slice 是从原始 ByteBuf 的 read index 到 write index 之间的内容进行切片，切**片后的 max capacity 被固定为这个区间的大小**，因此不能追加 write
        ...
        try {
            ...
            serverBootstrap.childHandler(new ChannelInitializer<SocketChannel>() {
                ...
                protected void initChannel(SocketChannel ch) throws Exception {
                    ch.pipeline().addLast(new LoggingHandler(LogLevel.DEBUG));
                    ch.pipeline().addLast(new ChannelInboundHandlerAdapter() {
                        @Override // 会在channel建立成功之后触发active事件
